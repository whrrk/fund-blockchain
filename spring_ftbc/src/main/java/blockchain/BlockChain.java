		...
		for(int i=1;i<blockChain.size();i++) {
			...
			for(int t=0; t<currentBlock.transactions.size();t++) {
				...
				for(Input input : currentTransaction.inputs) {
					...
					System.out.println("##"+i+"번째 블록의 "+t+"번 째  트랜잭션의 input.outputId : "+input.outputId);
					Object keys[] = tempUTXOs.keySet().toArray();
					...
					System.out.println(i+"반째 블록의 지운 input.outputId : "+input.outputId);
					tempUTXOs.remove(input.outputId);
				}
				for(Output output: currentTransaction.outputs) {
					System.out.println(i+"번째 블록의 tempUTXOs의 put 해준 output.id : "+output.id);
