...
<body>
		...
		function payment() {
			...
			IMP.request_pay({
				...
				if (rsp.success) {
					...
					location.href = "/chain/addTransaction.ftbc?imp_uid="
							+ rsp.imp_uid + "&paid_amount=" + rsp.paid_amount
							+ "&merchant_uid=" + rsp.merchant_uid
							+ "&apply_num=" + rsp.apply_num + "&buyer_email="
							+ rsp.buyer_email + "&buyer_name=" + rsp.buyer_name
							+ "&buyer_tel=" + rsp.buyer_tel + "&p_gift_code="
							+ rsp.name + "&p_title=" + p_title + "&pritxt="
							+ pritxt;
				} else {
					var msg = '결제에 실패하였습니다.';
	...
	<div class="container-fluid">
		...
		<div class="pay_row pay_box">
			<label>구매자 전화번호 - 필수입력정보</label> <input type="text" id="buyer_tel"
				placeholder=" 전화번호는 -를 뺀 숫자만 입력해주세요." style="font-size: 13px">
		</div>

		<div class="pay_row pay_box">
			<label>구매자 주소</label> <input type="text" id="buyer_addr"
				class="postcodify_address">
		</div>

		<div class="pay_row pay_box">
			<label>구매자 우편번호</label> <br> <input type="text" size="4"
				class="postcodify_postcode6_1 postcode" id="buyer_post1" /> &ndash;
			<label></label><input type="text" size="4"
